...
void speechContest(int index, vector<int>&v1, map<int,Speaker>&m, vector<int>&v2){
    multimap<int, int, greater<int>> groupMap; // key 分数  value 编号  greater<int>倒序排列
    int num = 0;
    for (vector<int>::iterator it = v1.begin(); it != v1.end();it++){
        ...
        //将平均分 放入到m容器中
        m[*it].m_Score[index - 1] = avg; // it 为选手编号
...
void MyPrintPerson2( map<int, Speaker>::reference m){
    cout << "编号: " << m.first << "  姓名 " <<m.second.m_Name << endl;
}
...
//    }
//    for_each(m.begin(), m.end(), MyPrintPerson2);

    //抽签
    speechDraw(v1);
    vector<int>v2; // 进入下一轮比赛的人员编号
    //比赛
    speechContest(1, v1 ,m ,v2); // index 代表第几轮比赛
    //显示比赛结果
